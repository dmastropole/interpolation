...
load XYGrids.mat;
load('/Users/Dana/Documents/MATLAB/Latrabjarg/XYZMod/XYZ_mod.mat');
...
for i=1:length(grids)
    ...
    %create grids
    if grd(i)==1
    var=create_grids(XYZ(i).xyz,'low',t_pvar(i),t_sig(i),[x_range y_range],...
        filename,LatBat);
    x=X.low(1,:);
    ...
    elseif grd(i)==2
    var=create_grids(XYZ(i).xyz,'med',t_pvar(i),t_sig(i),[x_range y_range],...
        filename,LatBat);
    x=X.med(1,:);
    dist=XYZ(i).fields.dist;
    [new_dist,ids]=nearest_neighbor(x,dist);
    ...
    elseif grd(i)==3
    var=create_grids(XYZ(i).xyz,'high',t_pvar(i),t_sig(i),[x_range y_range],...
        filename,LatBat);
    x=X.high(1,:);
    dist=XYZ(i).fields.dist;
    [new_dist,ids]=nearest_neighbor(x,dist);
